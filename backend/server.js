...
app.get("/api/download-form16", async (req, res) => {
  try {
    ...
    const emp = await Employee.findOne({ email: usr.email });
    ...
    if (!payrollData) return res.status(404).send("Payroll data not found");

    const taxableIncome = payrollData.net;
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=Form16_${month}_${year}.pdf`
    );
    doc.pipe(res);
    ...
    doc.moveDown(0.5);
    doc.text(`Employer: DataSkience (OPC) Private Limited`);
    ...
    doc.text(
      `Joining Date: ${emp.joinDate ? emp.joinDate.toDateString() : "-"}`
    );
    doc.moveDown();
    ...
    doc.moveDown(0.5);
    doc.text(`Basic Pay: ${payrollData.basic} /-`);
    doc.text(`Allowance: ${payrollData.allowance} /-`);
    doc.text(`Deductions: ${payrollData.deduction} /-`);
    doc.text(`Gross Salary: ${payrollData.gross} /-`);
    doc.text(`Net Salary: ${payrollData.net} /-`);
    doc.text(`Taxable Income: ${taxableIncome} /-`);
    ...
    doc.moveDown(0.5);
    doc.text(
      `Certified that ${taxPayable} /- has been deducted and deposited with the Income Tax Department.`
    );

    doc.moveDown(3);
    doc.fontSize(10).fillColor("#888").text("Generated by Payroll Management System", {
      align: "center",
    });
